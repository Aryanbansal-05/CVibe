    ...
    return (
        <div className="flex flex-wrap gap-5 py-5 w-[100%]">
            ...
            <div className="flex flex-col w-full gap-1">
                <label htmlFor={`profilePic-${info.id}`}>Upload Profile Picture</label>
                <input
                    type="file"
                    id={`profilePic-${info.id}`}
                    accept="image/*"
                    onChange={(e) =>
                        updateInfo("Personal", info.id, "profilePic", e.target.files[0])
                    }
                    className="file:rounded-md file:px-10 file:py-2 file:bg-blue-400 file:text-white file:border-none file:cursor-pointer file:transition hover:file:bg-blue-600 bg-gray-200 border-gray-200 rounded-md"
                />
            </div>
